...
(async () => {
    const settings = { ActorLookupCacheCapacity: 17, ActorLookupCacheTtlMinutes: 2, ActorLookupConcurrency: 3 };
    for (const values of [settings, { ...settings, ActorLookupCacheCapacity: 0 }]) {
    ...
    console.log('Actor settings form numeric save/reload and disable checks passed.');
